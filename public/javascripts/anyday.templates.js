...
angular.module("fixtures.jade", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("fixtures.jade",
    "<div id=\"fixtures\" layout=\"column\" ng-controller=\"AnyFixturesController as vm\"><md-autocomplete md-floating-label=\"do something...\" md-autoselect=\"true\" md-select-on-match=\"false\" md-no-cache=\"true\" md-delay=\"150\" md-selected-item=\"vm.fixture\" md-search-text=\"vm.search_text\" md-min-length=\"0\" md-items=\"fixture in vm.get_matches(vm.search_text)\" md-item-text=\"fixture.name\"><md-item-template><span md-highlight-text=\"vm.search_text\">{{ fixture.name }}</span></md-item-template></md-autocomplete><div layout layout-sm=\"column\"><md-input-container flex><label>every {{ vm.fixture.frequency | default:\"x\" }} days</label><input any-number type=\"number\" name=\"frequency\" ng-disabled=\"!vm.fixture\" ng-model=\"vm.fixture.frequency\"></md-input-container><md-datepicker ng-model=\"vm.fixture.date\" md-placeholder=\"Future Date\"></md-datepicker></div><div layout=\"row\" layout-align=\"end center\" class=\"md-actions\"><md-button ng-disabled=\"!vm.fixture\" ng-click=\"vm.create_from_fixture()\">Add</md-button></div></div>");
}]);
...
angular.module("tasks.jade", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("tasks.jade",
    "<div id=\"tasks\" ng-controller=\"AnyTasksController as vm\"><md-list><md-list-item ng-repeat=\"task in vm.tasks | orderBy:'time_left'\" ng-click=\"vm.show_bottom_sheet($event, task)\" class=\"md-2-line\"><div layout=\"row\" class=\"md-list-item-text\"><h4 style=\"width: 90px\">{{ vm.get_timestamp(task.time_left, $last) }}</h4><div layout=\"column\"><h3>{{ task.name }}</h3><div layout=\"row\"><h4>{{ task.when.relative() }}</h4><h4>| every {{ task.frequency }} days</h4><h4>| completed {{ task.analytics.length }} times</h4></div></div></div><md-icon aria-label=\"Update Time\" md-font-icon=\"material-icons\" ng-click=\"vm.update_time(task, 5)\" class=\"md-secondary\">check</md-icon></md-list-item></md-list></div>");
}]);
