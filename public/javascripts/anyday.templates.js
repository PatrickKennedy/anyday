...
angular.module("tasks.jade", []).run(["$templateCache", function($templateCache) {
  $templateCache.put("tasks.jade",
    "<div id=\"tasks\" ng-controller=\"AnyTasksController as vm\"><md-list><md-list-item ng-repeat=\"task in vm.tasks | orderBy:'time_left'\" ng-click=\"vm.show_bottom_sheet($event, task)\" class=\"md-2-line\"><div layout=\"row\" class=\"md-list-item-text\"><h4 style=\"width: 90px\">{{ vm.get_timestamp(task.time_left, $last) }}</h4><div layout=\"column\"><h3>{{ task.name }}</h3><div layout=\"row\"><h4>{{ task.when.relative() }}</h4><h4>| every {{ task.frequency }} days</h4></div></div></div><md-icon aria-label=\"Update Time\" md-font-icon=\"material-icons\" ng-click=\"vm.update_time(task, 5)\" class=\"md-secondary\">check</md-icon></md-list-item></md-list></div>");
}]);
